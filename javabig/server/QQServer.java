...
//	}
	...
	public void dealWithMessage(Flag flag, String message,TCP tcp)
	{
		switch(flag)
		{
			case LOGIN:doLogin(message,tcp);break;//处理用户登录事件
			case GET_FRIEND_INFO:doGetFriendInfo(message,tcp);break;//处理用户发来的请求好友资料的事件
			case REGISTER:doRegister(message,tcp);break;   //注册事件
			case CHANGE:doChange(message,tcp);break;   //个人管理
			case QUN_CHAT:doQunChat(message,tcp);break;     //处理用户发送的群聊事件
	...
	public void doRegister(String message,TCP tcp)
	{
		...
		{
			birthday=Date.valueOf("2018-11-16");
			repost+="\n生日日期格式不正确，系统已给您设置成默认的生日：2018-11-16!";
		}
	...
	public String getAllUsersInfo()
	{
		...
		{
			if(users.getState().equals("-1")||users.getState().equals("4"))
			{
